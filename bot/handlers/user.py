...
@user_router.message(CommandStart(), ~TicketFilter())
...
async def process_ticket_appeal(message : Message):
    result = await send_message_with_id(text= message.text, sender= Sender.user, id= message.from_user.id, ticket_id= None)
